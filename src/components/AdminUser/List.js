...
import { deleteAdminUser, fetchAllAdminUsers } from "../../services/admin-user";
...
const List = () => {
  ...
  const onView = (adminUser) => {
    setViewModal(true);
    setModalData(adminUser);
  };

  const onEdit = (adminUser) => {
    setEditModal(true);
    setModalData(adminUser);
  };

  const onDelete = (adminUser) => {
    setDeleteId(adminUser.id);
  ...
  const onDeleteHandler = async () => {
    dispatch(startLoading());
    const response = await deleteAdminUser(deleteId);
    fetchData();
    if (response.success && response.data) {
      dispatch(stopLoading());
