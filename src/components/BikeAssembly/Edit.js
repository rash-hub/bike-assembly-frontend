...
import { updateBikeAssembly } from "../../services/bike-assembly";
import { fetchAllBikes, fetchBikes } from "../../services/bike";
  ...
  useEffect(() => {
    (async () => {
      if (modalData?.bikeId) {
        ...
        if (response.success) {
          const currentBike = response.data.find(
            (bike) => bike.id === modalData?.bikeId
          );
          setCurrentBike(currentBike);
        } else {
          enqueueSnackbar(response?.data, { variant: "error" });
        }
      }
    })();
  }, [modalData?.bikeId, bikes]);
                  ...
                  <Grid item md={12}>
                    ...
                    >
                      ...
                      <Select
                        ...
                        onBlur={handleBlur}
                      >
                        <MenuItem key={currentBike?.id} value={currentBike?.id}>
                          {currentBike?.name}
                        </MenuItem>
                        {bikes.map((bike) => (
