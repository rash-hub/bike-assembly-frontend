...
export const updateAdminUser = async (id, values) => {
  try {
    const response = await rootInstance.put(`admin-user/${id}`, values);
...
export const viewAdminUser = async (id) => {
  try {
    const response = await rootInstance.get(`admin-user/${id}`);
    if (response?.data?.success && response?.data?.data) {
      return {
        success: true,
        data: response?.data?.data,
...
export const deleteAdminUser = async (id) => {
  try {
    const response = await rootInstance.delete(`admin-user/${id}`);
    if (response?.data?.success) {
      return {
        success: true,
        data: response?.data,
...
export const createAdminUser = async (values) => {
  try {
    const response = await rootInstance.post(`admin-user/`, values);
    if (response?.data?.success) {
      return {
        success: true,
        data: response?.data?.message,
