...
export const updateEmployee = async (id, values) => {
  try {
    const response = await adminRootInstance.put(`employee/${id}`, values);
...
export const viewEmployee = async (id) => {
  try {
    const response = await adminRootInstance.get(`employee/${id}`);
    if (response?.data?.success && response?.data?.data) {
      return {
        success: true,
        data: response?.data?.data,
...
export const deleteEmployee = async (id) => {
  try {
    const response = await adminRootInstance.delete(`employee/${id}`);
    if (response?.data?.success) {
      return {
        success: true,
        data: response?.data,
...
export const createEmployee = async (values) => {
  try {
    const response = await adminRootInstance.post(`employee/`, values);
    if (response?.data?.success) {
      return {
        success: true,
        data: response?.data?.message,
